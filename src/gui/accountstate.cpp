    ...
    , _isDesktopNotificationsAllowed(true)
{
    ...
    connect(this, &AccountState::isConnectedChanged, [this]{
        ...
        if (isConnected()) {
            ...
            // Start Files.fm messaging service polling when connected
            qCInfo(lcAccountState) << "Account connected, starting unlock message polling";
            if (_account->credentials() && _account->filesFmMessagingService()) {
                _account->filesFmMessagingService()->setUserAndToken(
                    _account->credentials()->user(),
                    _account->credentials()->password()
                );
                _account->filesFmMessagingService()->startUnlockMessagePolling();
            }
        } else {
            // Stop Files.fm messaging service polling when disconnected
            qCInfo(lcAccountState) << "Account disconnected, stopping unlock message polling";
            if (_account->filesFmMessagingService()) {
                _account->filesFmMessagingService()->stopUnlockMessagePolling();
            }
        }
    });
...
{
    // Stop Files.fm messaging service polling when signing out
    qCInfo(lcAccountState) << "Signing out, stopping unlock message polling";
    if (account()->filesFmMessagingService()) {
        account()->filesFmMessagingService()->stopUnlockMessagePolling();
    }
