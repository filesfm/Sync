                                     ...
                                     const QSize &requestedSize)
{
    ...
    // Check for colored version first (these should not be recolored)
    if (const auto coloredPath = QString(OCC::Theme::themePrefix + QStringLiteral("colored/") + fileName);
            QFile::exists(coloredPath)) {
        QSvgRenderer svgRenderer;
        if (!svgRenderer.load(coloredPath)) {
            qCWarning(lcIconUtils) << "Could not load colored SVG" << coloredPath;
            return {};
        }

        const auto reqSize = sizeToUse.isValid() && sizeToUse.width() > 0 && sizeToUse.height() > 0
                             ? sizeToUse
                             : svgRenderer.defaultSize();

        if (originalSize) {
            *originalSize = svgRenderer.defaultSize();
        }

        QImage image(reqSize, QImage::Format_ARGB32_Premultiplied);
