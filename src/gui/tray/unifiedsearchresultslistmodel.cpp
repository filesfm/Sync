...
QVariant UnifiedSearchResultsListModel::data(const QModelIndex &index, int role) const
{
    ...
    case SyncDirectoriesRole:
        return buildRemoteToLocalMap(_accountState);
    }
...
QHash<int, QByteArray> UnifiedSearchResultsListModel::roleNames() const
{
    ...
    roles[SyncDirectoriesRole] = "syncDirectories";
...
void UnifiedSearchResultsListModel::setSearchTerm(const QString &term)
{
    ...
    if (!_searchTerm.isEmpty()) {
        // Show an error message if the search term is too short
        if (_searchTerm.trimmed().length() < 2) {
            // Don't show error immediately, wait for timer
...
void UnifiedSearchResultsListModel::resultClicked(const QString &remotePath) const
{
    // Open the file corresponding to a remote path. Prefer direct remote->local resolution like
    // revealInFileManager(); fall back to legacy heuristics and URL opening.
    ...
    // Try to resolve remote -> local quickly using the shared helper.
    const QString localFullPath = resolveLocalFullPathForRemote(remotePath, _accountState);
    if (!localFullPath.isEmpty()) {
        qCInfo(lcUnifiedSearch) << "Opening file:" << localFullPath;
        QDesktopServices::openUrl(QUrl::fromLocalFile(localFullPath));
        return;
    }
}
...
QVariantMap buildRemoteToLocalMap(AccountState *accountState)
{
    QVariantMap remoteToLocal;
    const auto folders = FolderMan::instance()->map().values();
    for (auto *f : folders) {
        if (!f)
            continue;
        if (!accountState || f->accountState() == accountState)
            remoteToLocal.insert(f->remotePath(), f->path());
    }
    return remoteToLocal;
}

QString resolveLocalFullPathForRemote(const QString &path, AccountState *accountState)
{
    if (path.isEmpty()) {
        return QString();
    }

    QVariantMap remoteToLocal = buildRemoteToLocalMap(accountState);

    QString matchedRemote;
    for (auto it = remoteToLocal.constBegin(); it != remoteToLocal.constEnd(); ++it) {
        const QString remoteKey = it.key();
        const QString normalizedRemotePath = remoteKey.endsWith(QLatin1Char('/')) ? remoteKey : remoteKey + QLatin1Char('/');
        if (path.startsWith(normalizedRemotePath)) {
            if (matchedRemote.isEmpty() || remoteKey.length() > matchedRemote.length()) {
                matchedRemote = remoteKey;
            }
        }
    }
    ...
    QString localBase = remoteToLocal.value(matchedRemote).toString();
    QString relative = path.mid(matchedRemote.length());
    if (relative.startsWith(QLatin1Char('/'))) {
        relative = relative.mid(1);
    }

    QString localFullPath = localBase;
    if (!localFullPath.endsWith(QLatin1Char('/'))) {
        localFullPath += QLatin1Char('/');
    }
    if (!relative.isEmpty()) {
        localFullPath += relative;
    }

    return localFullPath;
}

void UnifiedSearchResultsListModel::revealInFileManager(const QString &remotePath) const
{
    if (remotePath.isEmpty()) {
        return;
    }
    ...
    showInFileManager(localFullPath);
}
...
void UnifiedSearchResultsListModel::slotSearchTermEditingFinished()
{
    ...
    // Show error message if search term is too short (ignoring whitespace)
    if (_searchTerm.trimmed().length() < 2) {
        _errorString = tr("Please enter at least 2 characters to search");
        emit errorStringChanged();
        return;
    }
...
void UnifiedSearchResultsListModel::parseResultsForProvider(const QJsonObject &data, const QString &providerId, bool fetchedMore)
{
    ...
    for (const auto &entry : entries) {
        ...
        // Check if the file is virtual (doesn't exist locally)
        // If it's virtual, add .filesfm suffix to indicate it's a virtual file
        const QString localFullPath = resolveLocalFullPathForRemote(result._subline, _accountState);
        if (localFullPath.isEmpty() || !QFile::exists(localFullPath)) {
            // Virtual file - add .filesfm suffix
            if (!result._title.endsWith(QLatin1String(".filesfm"))) {
                result._title.append(QLatin1String(".filesfm"));
            }
            if (!result._subline.endsWith(QLatin1String(".filesfm"))) {
                result._subline.append(QLatin1String(".filesfm"));
            }
        }
