...
{
    // Notification polling disabled - backend doesn't support it
    qCDebug(lcActivity) << "Notification polling is disabled";
    Q_UNUSED(interval);
...
{
    // Notification requests disabled - backend doesn't support them
    return;
...
void User::processCompletedSyncItem(const Folder *folder, const SyncFileItemPtr &item)
{
    ...
    const auto fileActionFromInstruction = [&item](const int instruction) {
        if (instruction == CSYNC_INSTRUCTION_REMOVE) {
            ...
            // Distinguish between rename (filename changed) and move (path changed)
            const auto sourceFileName = QFileInfo(item->_file).fileName();
            const auto targetFileName = QFileInfo(item->_renameTarget).fileName();

            if (sourceFileName != targetFileName) {
                // Filename changed - this is a rename (regardless of path)
                return QStringLiteral("file_renamed");
            } else {
                // Filename is the same, only path changed - this is a move
                return QStringLiteral("file_moved");
            }
        } else {
    ...
    const auto messageFromFileAction = [](const QString &fileAction, const QString &fileName) {
        ...
        } else if (fileAction == QStringLiteral("file_moved")) {
            return QObject::tr("You moved %1").arg(fileName);
    ...
    const auto messageFromFileActionServer = [](const QString &fileAction, const QString &fileName) {
        if (fileAction == QStringLiteral("file_renamed")) {
            return QObject::tr("Remote renamed %1").arg(fileName);
        } else if (fileAction == QStringLiteral("file_moved")) {
            return QObject::tr("Remote moved %1").arg(fileName);
        } else if (fileAction == QStringLiteral("file_deleted")) {
            return QObject::tr("Remote deleted %1").arg(fileName);
        } else if (fileAction == QStringLiteral("file_created")) {
            return QObject::tr("Remote created %1").arg(fileName);
        } else {
            return QObject::tr("Remote changed %1").arg(fileName);
        }
    };
    ...
    // Set the target file for rename and move operations
    if (activity._fileAction == QStringLiteral("file_renamed") || activity._fileAction == QStringLiteral("file_moved")) {
    ...
    if (item->_status == SyncFileItem::NoStatus || item->_status == SyncFileItem::Success) {
        ...
        if (item->_direction != SyncFileItem::Up) {
            activity._message = messageFromFileActionServer(activity._fileAction, fileName);
        } else {
...
{
    QString url = QString("https://files.fm/filebrowser");
