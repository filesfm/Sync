...
QVariant ActivityListModel::data(const QModelIndex &index, int role) const
{
    ...
    const auto getFilePath = [&]() {
        const auto fileName = (a._fileAction == QStringLiteral("file_renamed") || a._fileAction == QStringLiteral("file_moved")) ? a._renamedFile : a._file;
        if (!fileName.isEmpty()) {
    ...
    const auto generateIconPath = [&]() {
        ...
        if (a._type == Activity::NotificationType && !a._talkNotificationData.userAvatar.isEmpty()) {
            ...
            if (a._syncFileItemStatus == SyncFileItem::NormalError
                ...
                } else if (a._fileAction == "file_moved" || a._fileAction == "file_renamed") {
                    // Move and rename always use the monochrome change.svg (from black folder)
                    return colorIconPath.arg("change.svg");
                } else {
                    // Other changes (edits) use the colored change-bordered.svg when there's a preview
...
void ActivityListModel::slotTriggerDefaultAction(const int activityIndex)
{
    ...
    if (!path.isEmpty()) {
        ...
        if (fileInfo.isDir()) {
            ...
            // Open the file in the file manager and select it
