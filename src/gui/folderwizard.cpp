    ...
    , _account(account)
{
    ...
    QString defaultPath = QDir::homePath() + QLatin1Char('/') + Theme::instance()->appName() + QLatin1String(" - ") + _account->credentials()->user();
...
void FolderWizardSelectiveSync::initializePage()
{
    ...
    if (_virtualFilesCheckBox) {
        ...
        if (Utility::isPathWindowsDrivePartitionRoot(wizard()->field(QStringLiteral("sourceFolder")).toString())) {
            ...
            _virtualFilesCheckBox->setText(tr("Use virtual files instead of downloading content immediately %1").arg(bestAvailableVfsMode() == Vfs::WindowsCfApi ? tr("(recommended)") : tr("(experimental)")));
