...
void AccountManager::addAccountState(AccountState *const accountState)
{
    ...
    // Notify when the account transitions to a signed-out state.
    auto showReauth = [accountState]() {
        QMessageBox::information(nullptr,
                                 QObject::tr("Authentication required"),
                                 QObject::tr("Your session for account %1 has expired. Please sign in again.")
                                     .arg(accountState->account()->displayName()));
    };

    bool wasSignedOut = accountState->isSignedOut();

    // Show only on transitions to signed out, avoid repeated notifications
    connect(accountState, &AccountState::stateChanged, this, [accountState, showReauth, wasSignedOut](AccountState::State) mutable {
        const bool signedOut = accountState->isSignedOut();
