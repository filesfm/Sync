...
QUrl SocketApi::createLink(const QString &localFile, const QString command)
{
    auto fileData = FileData::get(localFile);
    QString account = fileData.folder->accountState()->account()->credentials()->user();
    const QString link = QStringLiteral("https://api.files.fm/server_scripts/filesfm_sync_contextmenu_action.php?username=%1&action=%2&path=%3")
...
void SocketApi::command_COPY_PRIVATE_LINK(const QString &localFile, SocketListener *)
{
    ...
    QUrl link = createLink(localFile, command);
    ...
    connect(reply, &QNetworkReply::finished, this, [reply, this]() {
        ...
        if (parseError.error != QJsonParseError::NoError || !doc.isObject()) {
            qCWarning(lcSocketApi) << "Failed to parse JSON response for private link:" << parseError.errorString()
                                   << "Raw response:" << QString::fromUtf8(response);
            return;
        }
        ...
        if (obj.contains("data") && obj.value("data").isObject()) {
            ...
            if (dataObj.contains("share_url") && dataObj.value("share_url").isObject()) {
                QJsonObject shareUrlObj = dataObj.value("share_url").toObject();
                if (shareUrlObj.contains("view") && shareUrlObj.value("view").isString()) {
                    shareUrl = shareUrlObj.value("view").toString();
                }
            }
        }
        if (!shareUrl.isEmpty()) {
            setClipboardText(shareUrl);
        } else {
            qCWarning(lcSocketApi) << "share_url.view not found in JSON response for private link. Full JSON:" << QString::fromUtf8(QJsonDocument(obj).toJson(QJsonDocument::Compact));
        }
    });
}
...
void SocketApi::command_ASK_TO_UNLOCK_FILE(const QString &localFile, SocketListener *listener)
{
    Q_UNUSED(listener)
    ...
    shareFolder->accountState()->account()->filesFmMessagingService()->sendUnlockRequest(QStringList{fileData.serverRelativePath});
}
...
void SocketApi::sendSharingContextMenuOptions(const FileData &fileData, SocketListener *listener, SharingContextItemEncryptedFlag itemEncryptionFlag, SharingContextItemRootEncryptedFolderFlag rootE2eeFolderFlag)
{
    ...
    if (itemEncryptionFlag == SharingContextItemEncryptedFlag::NotEncryptedItem) {
        listener->sendMessage(QLatin1String("MENU_ITEM:COPY_PRIVATE_LINK") + flagString + tr("Copy share link"));
    }
                                               ...
                                               const OCC::SocketListener* const listener) const
{
    if (!FileSystem::isDir(fileInfo.absoluteFilePath()) && syncFolder->accountState()->account()->capabilities().filesLockAvailable()) {
        if (syncFolder->accountState()->account()->fileLockStatus(syncFolder->journalDb(), fileData.folderRelativePath) == SyncFileItem::LockStatus::UnlockedItem) {
            // If the file is unlocked, it's either unlocked or locked by the current user. In any case, we can attempt to unlock it, if necessary.
            listener->sendMessage(QLatin1String("MENU_ITEM:UNLOCK_FILE::") + tr("Unlock file"));
        } else {
            // If the file is locked, we can ask the user to unlock it.
            listener->sendMessage(QLatin1String("MENU_ITEM:ASK_TO_UNLOCK_FILE::") + tr("Ask to unlock file"));
        }
    }
}
                                            ...
                                            const SyncJournalFileRecord &record) const
{
    ...
    if (!FileSystem::isDir(fileInfo.absoluteFilePath()) && syncFolder->accountState()->account()->capabilities().filesLockAvailable() &&
        ...
        //listener->sendMessage(QLatin1String("MENU_ITEM:LOCKED_FILE_OWNER:d:") + tr("Locked by %1").arg(record._lockstate._lockOwnerDisplayName.isEmpty() ? "unknown" : record._lockstate._lockOwnerDisplayName));
        //const auto lockExpirationTime = record._lockstate._lockTimeout;
        ...
        //listener->sendMessage(QLatin1String("MENU_ITEM:LOCKED_FILE_DATE:d:") + tr("Expires at %1", "time at which lock expires", lockExpirationTime).arg(lockExpirationTime ? QDateTime::fromSecsSinceEpoch(lockExpirationTime).toString("yyyy-MM-dd HH:mm") : "unknown"));
    }
}
