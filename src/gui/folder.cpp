    ...
    , _vfs(vfs.release())
{
...
#ifdef Q_OS_WIN
    _lockWatcherThread = new QThread(this);
    _lockWatcherThread->setObjectName("LockWatcherThread");
    ::LockWatcher* lockWatcher = new ::LockWatcher(this->path());
    lockWatcher->moveToThread(_lockWatcherThread);
    connect(_lockWatcherThread, &QThread::started, lockWatcher, &::LockWatcher::run);
    connect(_lockWatcherThread, &QThread::finished, lockWatcher, &QObject::deleteLater);
    connect(_lockWatcherThread, &QThread::finished, _lockWatcherThread, &QObject::deleteLater);
    _lockWatcherThread->start();

    connect(lockWatcher, &::LockWatcher::fileLocked, this, [this](const QString &file) {
        ...
        if (!canLockFile) {
            qWarning(lcFolder) << "Skipping locking file" << file << "with rec.isValid():" << rec.isValid();
            return;
        }
    ...
    connect(lockWatcher, &::LockWatcher::fileUnlocked, this, [this](const QString &file) {
        if (!file.startsWith(this->path())) {
            return;
        }
        ...
        const auto canUnlockFile = journalDb()->getFileRecord(fileRecordPath, &rec) && rec.isValid();

        if (!canUnlockFile) {
            qCInfo(lcFolder) << "Skipping unlocking file" << file
        ...
        _fileLockSuccess = connect(_accountState->account().data(), &Account::lockFileSuccess, this, [this, remoteFilePath]() {
            ...
            qCDebug(lcFolder) << "Unlocking a file succeeded" << remoteFilePath;
            startSync();
        });
        _fileLockFailure = connect(_accountState->account().data(), &Account::lockFileError, this, [this, remoteFilePath](const QString &message) {
...
{
#ifdef Q_OS_WIN
    if (_lockWatcherThread) {
        _lockWatcherThread->requestInterruption();
        _lockWatcherThread->quit();
        _lockWatcherThread->wait();
    }
#endif
...
void Folder::slotFilesLockReleased(const QSet<QString> &files)
{
    ...
    for (const auto &file : files) {
        ...
        // To prevent some weird behaviour, we will try to unlock ALL files
        const auto canUnlockFile = isFileRecordValid;
            ...
            // && rec._lockstate._lockOwnerId == _accountState->account()->credentials()->user();
