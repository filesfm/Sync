...
public:
    // For lenient credential invalidation (exponential backoff)
    int _consecutiveAuthFailures = 0;
    qint64 _authFailureStartTime = 0;
...
private:
    ...
    // Pointer to Files.fm Messaging service instance
    class Messaging* _filesFmMessagingService = nullptr;

public:
    // Set the Files.fm Messaging service for this account
    void setFilesFmMessagingService(class Messaging* service) { _filesFmMessagingService = service; }
    // Get the Files.fm Messaging service for this account
    class Messaging* filesFmMessagingService() const { return _filesFmMessagingService; }
