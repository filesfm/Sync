    ...
    , _serverColor(Theme::defaultColor())
{
    ...
    // Create the Files.fm Messaging service instance
    _filesFmMessagingService = new Messaging();
                               ...
                               const SyncFileItem::LockOwnerType lockOwnerType)
{
    ...
    connect(job.get(), &LockFileJob::finishedWithError, this, [lockStatus, serverRelativePath, this](const int httpErrorCode, const QString &errorString, const QString &lockOwnerName) {
        ...
        if (httpErrorCode == LockFileJob::LOCKED_HTTP_ERROR_CODE) {
            QString html = tr("File %1 is already locked by %2.<br><br><span style='color:grey;'>To send %2 a notification asking to unlock the file, press the \"Ask to unlock\" button.", "file locked dialog")
                .arg(filePath, lockOwnerName);
            html += tr("<br>You will be notified when the file is unlocked. Once it is unlocked, please <b>close and reopen</b> the file to edit it.</span>");

            QMessageBox msgBox;
            msgBox.setWindowTitle(tr("File Unlock Request"));
            msgBox.setTextFormat(Qt::RichText);
            msgBox.setText(html);
            // Add "Ask to unlock" button
            QPushButton *askButton = msgBox.addButton(tr("Ask to unlock"), QMessageBox::ActionRole);
            msgBox.setStandardButtons(QMessageBox::Ok);
            // Connect askButton to sendUnlockRequest
            QObject::connect(askButton, &QPushButton::clicked, this, [this, serverRelativePath]() {
                qCWarning(lcAccount) << "Ask to unlock button clicked";
                if (_filesFmMessagingService) {
                    _filesFmMessagingService->sendUnlockRequest(QStringList{serverRelativePath});
                }
            });
