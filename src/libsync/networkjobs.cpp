...
{
    qCWarning(lcEtagJob) << "Request Etag of" << path() << "FINISHED WITH STATUS"
                      <<  replyStatusString() << "response:" << reply()->peek(reply()->bytesAvailable());

    // Šis izpildās reizi pusminūtē un pie unauthorized izraisa to logout. Varētu būt labs sākumpunkts kur sākt pētīt, kāpēc notiek tā statusa maiņa un kā to novērst
...
void SimpleApiJob::start()
{
    ...
    // HACK: TEMPORARY FIX for slow search requests when made to webdav.files.fm
    QUrl url;
    // Redirect search API requests to webdavapp.files.fm instead of account host
    if (path().startsWith("ocs/v2.php/search")) {
        url = QUrl(QString("https://webdavapp.files.fm/%1").arg(path()));
        url.setQuery(query);
    } else {
        url = Utility::concatUrlPath(account()->url(), path(), query);
    }
