...
void PropagateRemoteDelete::slotDeleteJobFinished()
{
    ...
    // Defer deletion of database records to allow next sync cycle to detect moves by inode
    // This fixes race condition where directory is moved during sync, causing it to be
    // detected as DELETE + NEW instead of RENAME, resulting in data loss
    if (!propagator()->_journal->markFileRecordPendingDelete(_item->_originalFile, _item->isDirectory())) {
        qCWarning(lcPropagateRemoteDelete) << "could not mark file as pending delete in local DB" << _item->_originalFile;
        done(SyncFileItem::NormalError, tr("Could not mark file record %1 as pending delete in local DB").arg(_item->_originalFile), ErrorCategory::GenericError);
        return;
    }
